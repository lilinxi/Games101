    ...
    for (int _x = min_xy[0]; _x <= max_xy[0]; _x++) {
        for (int _y = min_xy[1]; _y <= max_xy[1]; _y++) {
            ...
            if (insideTriangle(x, y, t.v)) {
                ...
                if (judge_and_set_depth(_x, _y, zp)) {
                    // TODO: Interpolate the attributes:
                    // auto interpolated_color
                    // auto interpolated_normal
                    // auto interpolated_texcoords
                    // auto interpolated_shadingcoords
                    Eigen::Vector3f color = interpolate(alpha, beta, gamma, t.color[0], t.color[1], t.color[2],
                                                        alpha + beta + gamma);
                    Eigen::Vector3f normal = interpolate(alpha, beta, gamma, t.normal[0], t.normal[1], t.normal[2],
                                                         alpha + beta + gamma);
                    Eigen::Vector2f texcorrds = interpolate(alpha, beta, gamma, t.tex_coords[0], t.tex_coords[1],
                                                            t.tex_coords[2],
                                                            alpha + beta + gamma);
                    Eigen::Vector3f shadingcoords = interpolate(alpha, beta, gamma, view_pos[0], view_pos[1],
                                                                view_pos[2],
                                                                alpha + beta + gamma);
                    ...
                    vertex_shader_payload vertex = {
                            Vector3f{_x, _y, zp}
                    };

                    fragment_shader_payload fragment = {
                            color,
                            normal.normalized(),
                            texcorrds,
                            this->texture ? &*this->texture : nullptr
                    };
                    fragment.view_pos = shadingcoords;
                    Eigen::Vector3f shaderVertex = this->vertex_shader(vertex);
                    Eigen::Vector3f shaderFragment = this->fragment_shader(fragment);

                    this->set_pixel(Vector2i{shaderVertex.x(), shaderVertex.y()}, shaderFragment);
                }
            }
        }
    }
