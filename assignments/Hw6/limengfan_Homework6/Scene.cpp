...
//    TODO 对比
#if 0
    double nearT = MAXFLOAT;
    for (Object *o:this->objects) {
        Intersection inter = o->getIntersection(ray);
        if (inter.distance < nearT) {
            nearT = inter.distance;
            intersec = inter;
        }
    }
#else
...
//        Vector3f tmp = hitPoint;
        switch (m->getType()) {
            ...
            default: {
                ...
                hitColor = lightAmt * (Vector3f(0.5, 0.5, 0.5) * m->Kd + specularColor * m->Ks);
                break;
            }
        }
    }
